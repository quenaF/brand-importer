...
const VARIABLE_PATTERN = /^var\(\s*--[^)]+\s*\)$/i;
function slug(value){return value.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');}

export function findUnresolvedFontReferences(observations){
 return (observations?.fonts??[])
  .map(raw=>String(raw.value??'').replace(/^['"]|['"]$/g,'').trim())
  .filter(family=>VARIABLE_PATTERN.test(family));
}
...
export function normalizeTypography(observations,evidence){
 ...
 return (observations?.fonts??[]).map((raw,index)=>{
  ...
  if(VARIABLE_PATTERN.test(family)) return null;
 ...
 }).filter(Boolean).filter(x=>x.family).sort((a,b)=>b.score-a.score);
}
